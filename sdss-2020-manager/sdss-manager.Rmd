...
Prior approaches are meant for **static** package versioning.  
  ...
  - Changes across machines and over time
...
**Step 2**: Compare package dependency networks over time and across users/machines  
...
1. `take_inventory` and `plot_inventory` visualize scope of vulnerabilities  
2. Projects are more vulnerable than just to changes in explicitly called packages  
3. Reducing the number of packages used from the start will help reduce vulnerabilities  
...
`compare_inventory` function: strategically-placed **MD5 checksums** to summarize pieces of the inventory, such as:  
...
- can quickly filter out packages and objects which are identical across inventories and versions  
...
**Conclusions**  
  - data science pipelines might be more vulnerable than you realize   
...
**Future Work**  
  ...
  - visuals to better compare two dependency trees  
    - how removing a package dependency would change dependency tree   
